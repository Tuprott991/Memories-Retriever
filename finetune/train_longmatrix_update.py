...
# -*- coding: utf-8 -*-
"""
Train LongMatrix: a lightweight lexical→dense projection retriever.

- SDPA (FlashAttention/Triton) with safe mask + automatic matmul fallback.
- Multi-head + fused QKV attention (ép FlashAttention tối ưu).
- bf16/fp16 autocast + GradScaler (fp16) + optional torch.compile.
- Gradient checkpointing + grad accumulation for low VRAM.
- In-batch negatives + EMA for better accuracy.
- Streaming distillation để tránh RAM spikes.
- Detailed W&B logging per-step (ret/lex/ent/dist/ortho/lr).
- NEW: Late-interaction (ColBERT-lite) với top-K token nổi bật cho query/doc.
- MSMARCO v2.1 dataset support from HuggingFace.
- Hard negatives mining using BM25/M3/combo methods from data.py.
- Full DDP (Distributed Data Parallel) support with torchrun.
"""
...
def setup_distributed():
    """Initialize distributed training environment for torchrun."""
    if not _HAS_DDP:
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        print("[Warning] RANK/WORLD_SIZE not set, assuming single process")
        rank = 0
        world_size = 1
        local_rank = 0
...
def load_msmarco_v2_from_hf(split: str = 'train', max_samples: Optional[int] = None, 
    ...
    pairs = []
    if streaming:
        for i, example in enumerate(tqdm(dataset, desc=f"Loading {split}")):
            query = example.get('query', '')
            passages = example.get('passages', [])
            if query and passages:
                # Get positive passage (marked with is_selected=1)
        ...
        for i in tqdm(range(total), desc=f"Loading {split}"):
            example = dataset[i]
            query = example.get('query', '')
            passages = example.get('passages', [])
            if query and passages:
...
class TripletDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(negs) == 0:
            ...
            cand = list(negs) + random.choices(negs, k=self.k - len(negs)) if negs else []
        return {'q': q, 'p': p, 'negs': cand}

def mine_hard_negatives_integrated(train_pairs: List[Tuple[str, str]], 
                                   neg_method: str, k_neg: int, args) -> List[Tuple[str, str, List[str]]]:
    """Mine hard negatives using methods from data.py.
    
    Args:
        train_pairs: List of (query, positive_passage) pairs
        neg_method: 'random', 'm3', 'bm25', or 'combo'
        k_neg: Number of negatives per sample
        args: Training arguments with config
    
    Returns:
        List of (query, positive, [neg1, neg2, ...]) triplets
    """
    print(f"[NegMining] Using method: {neg_method}, k_neg: {k_neg}")
    
    if neg_method == 'random':
        # Random negatives from passage pool
        positives = [p for _, p in train_pairs]
        triplets = []
        for q, p in tqdm(train_pairs, desc="Mining random negatives"):
            negs = []
            seen = {p}
            while len(negs) < k_neg:
                cand = random.choice(positives)
                if cand not in seen:
                    seen.add(cand)
                    negs.append(cand)
            triplets.append((q, p, negs))
        return triplets
    
    elif neg_method in ('m3', 'bm25', 'combo'):
        # Import negative mining functions from data.py
        ...
        try:
            ...
            print(f"[Error] Cannot import from data.py: {e}")
            print("[Warning] Falling back to random negatives")
            return mine_hard_negatives_integrated(train_pairs, 'random', k_neg, args)
        
        device_str = pick_device(args.device)
        
        if neg_method == 'm3':
            print(f"[NegMining] Using M3 semantic negatives (device={device_str})")
            emb_dir = os.path.join(args.output_dir, 'emb_cache') if args.save_embeds else None
            triplets = m3_negatives_chunked(
                train_pairs=train_pairs,
                k_neg=k_neg,
                model_name=args.m3_model,
                device=device_str,
                bs=args.m3_bs,
                pool_limit=args.m3_pool_limit,
                save_embeds_dir=emb_dir,
                chunk_size_encode=20000,
                margin=args.m3_margin,
            )
            return [(q, p, list(negs)) for q, p, *negs in triplets]
            ...
            if args.bm25_engine == 'pyserini':
                ...
                triplets = bm25_negatives_pyserini(
                    train_pairs=train_pairs,
                    k_neg=k_neg,
                ...
                triplets = bm25_negatives_rankbm25(
                    train_pairs=train_pairs,
                    k_neg=k_neg,
                    pool_limit=args.bm25_pool_limit,
                    margin=args.bm25_margin,
            ...
            print("[NegMining] Using COMBO: BM25 + M3")
            k_bm25 = args.k_neg_bm25 if args.k_neg_bm25 is not None else k_neg // 2
            k_m3 = args.k_neg_m3 if args.k_neg_m3 is not None else k_neg - k_bm25
            ...
            if k_bm25 > 0:
                if args.bm25_engine == 'pyserini':
                    ...
                    work_dir = os.path.join(args.output_dir, 'bm25_work')
                    os.makedirs(work_dir, exist_ok=True)
                    bm25_trips = bm25_negatives_pyserini(
                        train_pairs=train_pairs, k_neg=k_bm25,
                        pool_limit=args.bm25_pool_limit, index_dir=idx_dir,
                        work_dir=work_dir, language=args.bm25_language,
                        threads=args.bm25_threads, margin=args.bm25_margin,
                        k1=args.bm25_k1, b=args.bm25_b,
                    )
                else:
                    bm25_trips = bm25_negatives_rankbm25(
                        train_pairs=train_pairs, k_neg=k_bm25,
                        pool_limit=args.bm25_pool_limit, margin=args.bm25_margin,
                    )
            
            if k_m3 > 0:
                emb_dir = os.path.join(args.output_dir, 'emb_cache') if args.save_embeds else None
                m3_trips = m3_negatives_chunked(
                    train_pairs=train_pairs, k_neg=k_m3,
                    model_name=args.m3_model, device=device_str,
                    bs=args.m3_bs, pool_limit=args.m3_pool_limit,
                    save_embeds_dir=emb_dir, chunk_size_encode=20000,
                    margin=args.m3_margin,
                )
            
            # Merge negatives
            if bm25_trips and m3_trips:
                merged = []
                for (q1, p1, *nb), (q2, p2, *nm) in zip(bm25_trips, m3_trips):
                    seen = set()
                    negs = []
                    for cand in list(nb) + list(nm):
                        if cand not in seen and cand != p1:
                            seen.add(cand)
                            negs.append(cand)
                    merged.append((q1, p1, negs))
                return merged
            else:
                trips = bm25_trips or m3_trips
                return [(q, p, list(negs)) for q, p, *negs in trips]
...
def parse_args():
    ...
    ap.add_argument('--train_tsv', type=str, default=None, help='Path to train TSV (for tsv mode)')
    ap.add_argument('--dev_tsv', type=str, default=None, help='Path to dev TSV (for tsv mode)')
    ...
    # MSMARCO options
    ap.add_argument('--msmarco_train_split', type=str, default='train', help='MSMARCO train split name')
    ap.add_argument('--msmarco_dev_split', type=str, default='validation', help='MSMARCO dev split name')
    ap.add_argument('--msmarco_streaming', action='store_true', help='Use streaming mode for MSMARCO')
    ap.add_argument('--hf_cache_dir', type=str, default=None, help='HuggingFace cache directory')

    # Negative mining options (from data.py)
    ap.add_argument('--neg_method', type=str, default='random', choices=['random', 'm3', 'bm25', 'combo'],
                    help='Method for mining hard negatives')
    ap.add_argument('--k_neg', type=int, default=7, help='Number of negatives per query')
    
    # M3 negative mining options
    ap.add_argument('--m3_model', type=str, default='BAAI/bge-m3', help='M3 model for semantic negatives')
    ap.add_argument('--m3_bs', type=int, default=64, help='M3 encoding batch size')
    ap.add_argument('--m3_pool_limit', type=int, default=100000, help='Limit positives for M3 index')
    ap.add_argument('--m3_margin', type=int, default=8, help='Extra candidates over k_neg for M3')
    ap.add_argument('--save_embeds', action='store_true', help='Cache M3 embeddings to disk')
    
    # BM25 negative mining options
    ap.add_argument('--bm25_engine', type=str, default='rankbm25', choices=['pyserini', 'rankbm25'],
                    help='BM25 backend engine')
    ap.add_argument('--bm25_pool_limit', type=int, default=200000, help='Limit positives for BM25 index')
    ap.add_argument('--bm25_language', type=str, default='en', help='Language for BM25 analyzer (pyserini)')
    ap.add_argument('--bm25_threads', type=int, default=8, help='Indexing threads for BM25 (pyserini)')
    ...
    ap.add_argument('--k_neg_m3', type=int, default=None, help='Negatives from M3 in combo mode')
    ...
    ap.add_argument('--device', type=str, default='cuda', help='Device for training (cuda/cpu/auto)')
    ...
    ap.add_argument('--dev_eval_every_epochs', type=float, default=1.0,
                    help='Evaluate dev after X epochs (float) in optimizer steps')

    # Late interaction flags
    ap.add_argument('--late_interaction', action='store_true',
                    help='Enable ColBERT-lite: top-K token vectors for query/doc')
    ap.add_argument('--topk_q', type=int, default=4, help='Number of tokens K for query when late_interaction')
    ap.add_argument('--topk_d', type=int, default=1, help='Number of tokens K for doc when late_interaction')

    # DDP: Distributed Data Parallel for multi-GPU training with torchrun
...
def main():
    ...
    # Auto-detect DDP from environment (torchrun sets RANK/WORLD_SIZE)
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        args.ddp = True
    ...
    if args.ddp:
        ...
        print(f"[Rank {rank}/{world_size}] Initialized DDP", flush=True)
    ...
    if args.ddp:
        dist.barrier()  # Wait for main process to create directory

    # Save YAML config (only on main process)
    if is_main_process:
    ...
    if is_main_process:
        print(f'Device: {device} | dtype: {args.dtype} | DDP: {args.ddp} | World size: {world_size}')
    ...
    if args.data_source == 'msmarco':
        if is_main_process:
            print(f"[Data] Loading MSMARCO v2.1 from HuggingFace...")
        
        # Load train pairs
        train_pairs = load_msmarco_v2_from_hf(
            split=args.msmarco_train_split,
            max_samples=args.max_train_rows,
            cache_dir=args.hf_cache_dir,
            streaming=args.msmarco_streaming
        )
        
        # Load dev pairs
        if args.msmarco_dev_split:
            dev_pairs = load_msmarco_v2_from_hf(
                split=args.msmarco_dev_split,
                max_samples=args.dev_eval_sample,
                cache_dir=args.hf_cache_dir,
                streaming=args.msmarco_streaming
            )
        else:
        ...
        # Mine hard negatives (only on main process to avoid redundant work)
        if is_main_process:
            print(f"[NegMining] Mining hard negatives using {args.neg_method}...")
            train_rows = mine_hard_negatives_integrated(train_pairs, args.neg_method, args.k_neg, args)
            
            # Convert dev pairs to triplets (no negatives needed for evaluation)
            dev_rows = [(q, p, []) for q, p in dev_pairs] if dev_pairs else []
            
            # Save preprocessed data for other ranks
            if args.ddp:
                cache_train = os.path.join(args.output_dir, 'train_triplets_cache.json')
                cache_dev = os.path.join(args.output_dir, 'dev_triplets_cache.json')
                with open(cache_train, 'w', encoding='utf-8') as f:
                    json.dump(train_rows, f, ensure_ascii=False)
                with open(cache_dev, 'w', encoding='utf-8') as f:
                    json.dump(dev_rows, f, ensure_ascii=False)
                print(f"[Cache] Saved triplets to {cache_train} and {cache_dev}")
        
        if args.ddp:
            dist.barrier()  # Wait for main process to finish mining
            
            if not is_main_process:
                # Load from cache
                cache_train = os.path.join(args.output_dir, 'train_triplets_cache.json')
                cache_dev = os.path.join(args.output_dir, 'dev_triplets_cache.json')
                with open(cache_train, 'r', encoding='utf-8') as f:
                    train_rows = json.load(f)
                    train_rows = [(q, p, negs) for q, p, negs in train_rows]
                with open(cache_dev, 'r', encoding='utf-8') as f:
                    dev_rows = json.load(f)
                    dev_rows = [(q, p, negs) for q, p, negs in dev_rows]
                print(f"[Rank {rank}] Loaded triplets from cache", flush=True)
    
    else:  # TSV mode
        if not args.train_tsv:
            raise ValueError("--train_tsv required when --data_source=tsv")
    ...
    # Create dataset and dataloader with DDP support
    ...
    shuffle = True
    if args.ddp:
        sampler = DistributedSampler(ds, num_replicas=world_size, rank=rank, shuffle=True, seed=args.seed)
    ...
    if args.lambda_distill > 0.0:
        ...
        if is_main_process:
            print(f'Teacher dim: {args.m_teacher}')

    # Create model
    if is_main_process:
        print(f"[Model] Creating LongMatrixModel...")
    ...
    if args.ddp:
        ...
        if is_main_process:
            print(f"[DDP] Wrapped model with DDP")
    ...
    # EMA (only on main process to save memory)
    ema = None
    if args.ema and args.ema > 0.0 and is_main_process:
        ema = EMA(model.module if args.ddp else model, decay=args.ema)
    args._ema_obj = ema
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # DDP: Set epoch for sampler (ensures different shuffle each epoch)
        if args.ddp:
        ...
        metrics = None
        if dev_rows and len(dev_rows) > 0 and is_main_process:
            sample_n = None if args.dev_eval_sample in (-1, None) else args.dev_eval_sample
            
            # Use unwrapped model for evaluation
            eval_model = model.module if args.ddp else model
            
            if ema is not None:
                ema.apply_to(eval_model)
            
            metrics = evaluate_dev(eval_model, dev_rows, tokenizer, device, args, 
                                 max_len=args.max_len, topk=10, sample=sample_n)
            ...
            if args.wandb:
                ...
                wandb_log_table_examples(eval_model, tokenizer, device, dev_rows, args, 
                                       max_len=args.max_len, topk=5, nsamples=args.wandb_table_samples)
            ...
            if improved:
                ...
                torch.save({'model': eval_model.state_dict(), 'args': safe_args, 'metrics': metrics}, ckpt)
    ...
    # Export index (only on main process)
    if args.export_after_train and is_main_process:
        ...
        # Use unwrapped model for export
        export_model = model.module if args.ddp else model
        export_index(export_model, tokenizer, device, texts, export_dir, args.max_len, 
                    args.faiss_hnsw_m, args.faiss_hnsw_efc, args.faiss_search_efs, 
                    args.export_demo_query, args.export_demo_topk)

    # DDP cleanup
    if args.ddp:
        cleanup_distributed()
        if is_main_process:
            print("[DDP] Cleaned up distributed training")
